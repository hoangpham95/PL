...
Core evaluation rules:
  ...
  eval({rec {id E1} E2},env) = eval({with {id {call Y {fun id} E1 }}
                                    E2}, env)
  eval({fun {x} E},env)      = <{fun {x} E}, env>
  eval({call E1 E2},env1)    = eval(Ef,cons(eval(E2,env1),env2))
                               if eval(E1,env1) = <{fun {x} Ef}, env2>
                             = error!  otherwise
  eval({if E1 E2 E3},env)    = eval(E2,env) if eval(E1,env) = 0
                             = eval(E3,env) else
...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(cons 'rec more)
     (match sexpr
       [(list 'rec (list (symbol: name) fun) body)
        (let ([fun-var (parse-sexpr fun)])
          (cases fun-var
            [(Fun bound-ids bound-body)
             (WRec name fun-var (parse-sexpr body))]
    ...
    [(cons 'if0 more)
     (match sexpr
       [(list 'if0 f s t)
        (If0 (parse-sexpr f) (parse-sexpr s) (parse-sexpr t))]
       [else (error 'parse-sexpr
                    "bad `if0' syntax in ~s" sexpr)])]
...
(define (preprocess expr de-env)
  ...
  (cases expr
    ...
    [(WRec name fun body)
     (sub (With name
                (Call Y-combinator (list (Fun (list name) fun)))
                body))]
    [(Fun bound-ids bound-body)
    ...
    [(Call fun-expr arg-exprs)
     ...
     (if (= 1 (length arg-exprs))
       ...
       (sub (Call (Call fun-expr (list (first arg-exprs)))
                  (rest arg-exprs))))]
    [(If0 f s t) (CIf0 (sub f) (sub s) (sub t))]))

(: Y-combinator : BRANG)
;; make a function recursive
(define Y-combinator
  (parse "{fun {b}
              {call {fun {f} {call b {fun {x} {call {call f f} x}}}}
              {fun {f} {call b {fun {x} {call {call f f} x}}}}}}"))
     ...
     (let ([fval (eval fun-expr env)])
       (cases fval
         ...
         [else (error 'eval "`call' expects a function, got: ~s"
                            fval)]))]
    [(CIf0 f s t)
     (if (zero? (NumV->number (eval f env)))
         (eval s env)
         (eval t env))]))
...
(test (run "{rec {fact {fun {Y}
              ...
              {call fact 5}}")
      => 120)
(test (run "{rec {fib {fun {a b count}
                        {if0 count
                             b
