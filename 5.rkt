...
;; parses a string containing a PROGRAM expression to an PROGRAM AST
(define (parse str)
...
(define (lookup-fun name prog)
  (cases prog
    [(Funs funs)
     (let ([found (ormap (lambda ([fun : FUN]) 
                           (if (eq? name (cases fun 
                                           [(Fun id arg body) id]))
                               fun
                               #f))
                         funs)])
       (or found (error 'lookup-fun "Fun ~s not found in ~s" name prog)))]))
...
(test (run* "{and}") => #t)
(test (run* "{or}") => #f)
(test (run* "{and True False True}") => #f)
...
(test (run "{program {fun add1 {n} {+ 1 n}}
                     {fun main {n} {call add1 n}}}" 1) => 2)
(test (run "{program {fun add1 {n} {+ 1 n}}
                     {fun main {n} {vcall {quote add1} n}}}" 1) => 2)
(test (run "{program
               {fun even? {n}
                    {if {= 0 n} True {call odd? {- n 1}}}}
               {fun odd? {n}
