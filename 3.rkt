...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(list 'sqrt rad) (Sqrt (parse-sexpr rad))]
...
(define (subst expr from to)
  (cases expr
    ...
    [(Sqrt rad) (Sqrt (subst rad from to))]))
...
(define (sqrt+ ns)
  (cond [(null? ns) null]
        ...
        [else (let ([result (sqrt (first ns))])
                (cons result (cons (- result) (sqrt+ (rest ns)))))]))
...
(test (run "{with {x {* 5 5}} {+ x x}}") => '(50))
(test (run "{with {x {/ 5 5}} {+ x x}}") => '(2))
...
(test (run "{+ {/ {+ {sqrt 1} 3} 2} {sqrt {sqrt 0}}}") => '(2 2 2 2 1 1 1 1))
